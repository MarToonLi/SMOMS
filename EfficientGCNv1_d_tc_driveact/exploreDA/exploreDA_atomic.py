...
def make_dataset(task, split, mode):
    if task not in ["actionlevel", "objectlevel", "locationlevel", "alllevel"]:
    ...
    taskPath = os.path.join(basePath, "{}.csv".format(task))
    with open(taskPath) as f:
        reader = csv.reader(f)
        label_list = [row[1] for row in reader]

    split_file_Path = os.path.join(basePath,
                                   "inner_mirror/{}.chunks_90.split_{}.{}.csv".format("objectlevel", split, mode))
...
def csv2npy_main(task, split, mode):
    ...
    form_clip(task,label2id_dict, df_splitfile, skeletons_folder_name)


def form_clip(task,label2id_dict, df_splitfile, skeletons_folder_name):
    """
    1 提供
    ————————————————————————————————————
    1 首先记录总共的帧数，包含缺失帧；
    ...
    for participant_id in participant_id_lst:  # 10 drivers
        ...
        for file_id in file_id_lst:  # 1 2
            ...
            for i in tqdm(range(len(clip_2))):
                _, file_id, annotation_id, frame_start, frame_end, activity, object, location, chunk_id = clip_2.iloc[i,
                                                                                                          :]
                if task == "alllevel":
                    activity = "{}_{}_{}".format(activity, object, location)
                elif task == "actionlevel":
                    activity = activity
                elif task == "objectlevel":
                    activity = object
                elif task == "locationlevel":
                    activity = location
                else:
...
if __name__ == '__main__':
    ...
    # ["actionlevel", "objectlevel", "locationlevel", "alllevel"]:
    csv2npy_main(task="locationlevel", split=0, mode="train")
    csv2npy_main(task="locationlevel", split=0, mode="val")
    csv2npy_main(task="locationlevel", split=0, mode="test")
    csv2npy_main(task="locationlevel", split=1, mode="train")
    csv2npy_main(task="locationlevel", split=1, mode="val")
    csv2npy_main(task="locationlevel", split=1, mode="test")
    csv2npy_main(task="locationlevel", split=2, mode="train")
    csv2npy_main(task="locationlevel", split=2, mode="val")
    csv2npy_main(task="locationlevel", split=2, mode="test")
