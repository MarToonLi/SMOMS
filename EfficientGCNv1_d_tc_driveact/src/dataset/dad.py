...
class DAD_Feeder(Dataset):
    def __init__(self, phase, dataset_path, inputs, num_frame, connect_joint, debug, **kwargs):
        ...
        if os.path.exists(data_path) and os.path.exists(label_path):
            ...
            if "driveract" in dataset_path and "midlevel" in dataset_path:
                self.label = [int(label) - 2 for label in self.label]
            elif "driveract" in dataset_path and "midlevel" not in dataset_path:
        ...
        # if phase == "train":
        #     self.class_sample_count = np.array([len(np.where(self.label == t)[0]) for t in np.unique(self.label)])
        #     weight = 1. / self.class_sample_count
        #     print(len(weight))
        #     self.samples_weight = np.array([weight[t] for t in self.label])
        # else:
        #     self.samples_weight = None

    def get_samples_weight(self):
        # self.class_sample_count = np.array([len(np.where(self.label == t)[0]) for t in np.unique(self.label)])
        weight = {t: 1. / len(np.where(self.label == t)[0]) for t in np.unique(self.label)}
        # weight = {x:x for x in np.unique(self.label)}
        # logging.info("===========>", len(weight), len(np.unique(self.label)), max(np.unique(self.label)),
        #              min(np.unique(self.label)))
