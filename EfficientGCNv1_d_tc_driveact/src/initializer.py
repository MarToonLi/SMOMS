...
class Initializer():
    ...
    def init_dataloader(self):
        if "_" in self.args.dataset:
            dataset_name = self.args.dataset.split('-')[0]
        else:
            dataset_name = str(self.args.dataset).lower()
        dataset_args = self.args.dataset_args[dataset_name]
        dataset_args['debug'] = self.args.debug
        # suffix
        dataset_args['suffix'] = self.args.suffix

        dataset_args['Ad'] = self.args.Ad
        dataset_args['withself'] = self.args.withself
        logging.info('**> Ad:{} Withself: {}'.format(self.args.Ad, self.args.withself))

        self.train_batch_size = dataset_args['train_batch_size']
        self.eval_batch_size = dataset_args['eval_batch_size']
        self.test_batch_size = dataset_args['eval_batch_size']

        self.feeders, self.data_shape, self.num_class, self.A, self.parts, self.samples_weight = dataset.create(
            self.args.dataset, **dataset_args
        )

        samples_weight = torch.from_numpy(self.samples_weight)
        ...
        # todo:0425:shuffle turn to False and add the sampler
        self.eval_loader = DataLoader(self.feeders['eval'],
                                      batch_size=self.eval_batch_size, num_workers=4 * len(self.args.gpus),
                                      pin_memory=True, shuffle=False, drop_last=False
                                      )
        self.test_loader = DataLoader(self.feeders['test'],
                                      batch_size=self.test_batch_size, num_workers=4 * len(self.args.gpus),
                                      pin_memory=True, shuffle=False, drop_last=False
                                      )
    ...
    def init_model(self):
        ...
        self.args.model_args["layer_type"] = self.args.tcl
        self.args.model_args["expand_ratio"] = self.args.tcr
        self.args.model_args["reduct_ratio"] = self.args.tcr
        self.args.model_args["ratio"] = self.args.tcr

        try:
            self.args.model_args["kernel_size"] = [self.args.model_args["kernel_size"][0], self.args.gcmh]
            logging.info("**** GCNMH:{}".format(self.args.gcmh))
        except:
            logging.info("No Use GCNMH:{}")
            pass
        ...
        # logging.info('Model profile: {:.2f}G FLOPs and {}M Parameters'.format(flops / 1e9, params / 1e6))

        params_add = sum(p.numel() for p in self.model.parameters() if p.requires_grad)
        logging.info('params_add:{}'.format(params_add))
