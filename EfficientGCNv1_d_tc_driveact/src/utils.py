...
def load_checkpoint(work_dir, model_name='resume'):
    if model_name == 'resume':
            ...
            for i, dir_time in enumerate(os.listdir(work_dir)):
                if os.path.isdir('{}/{}'.format(work_dir, dir_time)):
                    ...
                    if os.path.exists(state_file):
                        ...
                        accs[str(i + 1)] = best_state['balanced_acc']
        ...
        for key in dirs.keys():
            print('({}) balanced accuracy: {:.2%} | training time: {}'.format(key, accs[key], dirs[key]))
            logging.info('({}) balanced accuracy: {:.2%} | training time: {}'.format(key, accs[key], dirs[key]))
