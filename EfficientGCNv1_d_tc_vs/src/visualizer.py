...
class Visualizer():
    ...
    def show_NTU_skeleton(self):
        if len(self.location) == 0:
            ...
            logging.info('')
            return
        # 似乎 Location_Feeder 做的事情是保存第一个人的信息和第二个人的颜色信息。
    ...
    def show_3MDAD_skeleton(self):
        ...
        C, T, V, M = self.location.shape
        print("self.location.shape：{}".format(self.location.shape))
            ...
            for m in range(M):
                ...
                for v in range(V):
                    ...
                    if x[v] == 0.0 or x[k] == 0.0:
                        continue
                    plt.plot([x[v], x[k]], [y[v], y[k]], '-o', c=np.array([0.1, 0.1, 0.1]), linewidth=linewidth,
                             markersize=0)
                    plt.scatter(x[v], y[v], marker='o', c=[r, g, b], s=jointwidth * 0.2 * (1 + r * 15))

                # plt.scatter(x, y, marker='o', c=c, s=jointwidth)
                # plt.scatter(x[2], y[2], marker='x', c="red", s=SpecialJointWidth)
                # plt.scatter(x[5], y[5], marker='x', c="red", s=SpecialJointWidth)
            plt.pause(pause)
        plt.ioff()
        plt.show()
