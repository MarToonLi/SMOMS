...
class EfficientGCN(nn.Module):
    ...
    def forward(self, x):
        ...
        # input branches
        # print("log:{},{}".format(x.shape, len(self.input_branches)))
...
class EfficientGCN_Blocks(nn.Sequential):
    def __init__(self, init_channel, block_args, layer_type, kernel_size, input_channel=0, **kwargs):
        ...
        if input_channel > 0:  # if the blocks in the input branches
            ...
            # params_add = sum(p.numel() for p in Spatial_Graph_Layer(input_channel, init_channel, max_graph_distance, **kwargs).parameters() if p.requires_grad)
            # print('==Spatial_Graph_Layer params_add:{}'.format(params_add))
            # params_add = sum(p.numel() for p in Temporal_Basic_Layer(init_channel, temporal_window_size, **kwargs).parameters() if p.requires_grad)
            # print('==Temporal_Basic_Layer params_add:{}'.format(params_add))

        last_channel = init_channel
        if "Mta" not in layer_type:
            logging.info("****> LayerType:{}".format(layer_type))
            ...
            logging.info(
                "****> LayerType:{} --> block_type:{} --> SubLayer:{}".format(layer_type, block_type, sub_layer_type))
        ...
        for i, [channel, stride, depth] in enumerate(block_args):
            ...
            # params_add = sum(p.numel() for p in
            #                  Spatial_Graph_Layer(last_channel, channel, max_graph_distance, **kwargs).parameters()
            #                  if p.requires_grad)
            # print('-{}-Spatial_Graph_Layer params_add:{}'.format(i,params_add))
            # for j in range(depth):
            #     s = stride if j == 0 else 1
            #     params_add = sum(
            #         p.numel() for p in temporal_layer(channel, temporal_window_size, stride=s, **kwargs).parameters() if
            #         p.requires_grad)
            #     print('-{}-Temporal_Basic_Layer params_add:{}'.format(i,params_add))
            # params_add = sum(
            #     p.numel() for p in Attention_Layer(channel, **kwargs).parameters() if
            #     p.requires_grad)
            # print('-{}-Attention_Layer params_add:{}'.format(i,params_add))
