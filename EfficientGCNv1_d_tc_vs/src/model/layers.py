...
class Temporal_Sep_Layer(nn.Module):
    def __init__(self, channel, temporal_window_size, bias, act, expand_ratio, stride=1, residual=True, **kwargs):
        ...
        padding = (temporal_window_size - 1) // 2
        self.act = act
        print(channel,expand_ratio,temporal_window_size)
        if expand_ratio > 0:
        ...
        i = 0
        params_add = sum(
            p.numel() for p in self.expand_conv.parameters() if
            p.requires_grad)
        print('-{}-expand_conv params_add:{}'.format(i, params_add))
        params_add = sum(
            p.numel() for p in self.depth_conv.parameters() if
            p.requires_grad)
        print('-{}-depth_conv params_add:{}'.format(i, params_add))
        params_add = sum(
            p.numel() for p in self.point_conv.parameters() if
            p.requires_grad)
        print('-{}-point_conv params_add:{}'.format(i, params_add))
        params_add = sum(
            p.numel() for p in self.residual.parameters() if
            p.requires_grad)
        print('-{}-residual params_add:{}'.format(i, params_add))
