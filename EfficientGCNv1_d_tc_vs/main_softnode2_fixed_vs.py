...
from src.visualizer_all import Visualizer
...
def main():
    ...
    elif args.datasetCode == "driveract":
        configyaml = "./config0316_softnode3_{}/train_driveract_{}_{}.yaml".format("mid", 0, 90)
    else:
        print(args.datasetCode)
        raise ValueError("error datasetCode!")
    args = update_parameters(parser, args, configyaml)  # cmd > yaml > default

    if args.datasetCode == "driveract":
        root_folder = args.dataset_args[args.dataset]["root_folder"]
        args.dataset_args[args.dataset]["root_folder"] = \
            str(root_folder).replace(r"driveract\{}".format(0),
                                     r"driveract\{}level\B_f{}_{}_{}".format("mid", 90, 0, 0))
    ...
    if args.configCode == 2561:
        ...
        args.scheduler_args["cosine"]["max_epoch"] = 80
        args.scheduler_args["cosine"]["warm_up"] = 0
        args.dataset_args["3mdad_downsampled1"]["train_batch_size"] = 32
        args.dataset_args["3mdad_downsampled1"]["train_batch_size"] = 32
        ...
        args.scheduler_args["cosine"]["warm_up"] = 5
    elif args.configCode == 1150:  # split 0
        args.seed = 1150
        args.optimizer_args["SGD"]["lr"] = 0.05
        args.optimizer_args["SGD"]["weight_decay"] = 0.0004
        args.scheduler_args["cosine"]["max_epoch"] = 70
    ...
    if args.generate_data:
        ...
        import os
        if args.dataset == "ebdd":
            out_images_folder_path = r"N:\clip225\VisualEBDD"
        elif args.dataset == "3mdad_downsampled1":
            out_images_folder_path = r"N:\3MDAD\VisualRGB1"
        elif args.dataset == "3mdad_downsampled2":
            out_images_folder_path = r"N:\3MDAD\VisualRGB2"
        elif args.dataset == "driveract":
            out_images_folder_path = r"N:\Driver&Act\data\VisualdDriveAct_smoothnode"
        else:
            raise ValueError("Error dataset!")

        if os.path.exists(out_images_folder_path):
            import shutil
            shutil.rmtree(out_images_folder_path)
        os.mkdir(out_images_folder_path)
        args.out_images_folder_path = out_images_folder_path
...
def init_parser():
    ...
    parser.add_argument('--extract', '-ex', default=True, action='store_true', help='Extract')
    parser.add_argument('--visualize', '-v', default=True, action='store_true', help='Visualization')
    ...
    # parser.add_argument('--configCode', type=int, default=2561, help='configCode')
    # parser.add_argument('--datasetCode', type=str, default="3mdad-rgb2", help='datasetCode')
    ...
    parser.add_argument('--configCode', type=int, default=1150, help='configCode')
    parser.add_argument('--datasetCode', type=str, default="driveract", help='datasetCode')

    # parser.add_argument('--configCode', type=int, default=9552, help='configCode')
    # parser.add_argument('--datasetCode', type=str, default="3mdad-rgb1", help='datasetCode')
    ...
    parser.add_argument('--out_images_folder_path', type=str, default='', help='Work dir')
