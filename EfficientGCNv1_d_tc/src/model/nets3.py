...
class EfficientGCN_Blocks(nn.Sequential):
    ...
    def __init__(self, init_channel, block_args, layer_type, kernel_size, input_channel=0, **kwargs):
        ...
        if "Mta" not in layer_type:
            ...
            temporal_layer = U.import_class(f'src.model.layers_temporal.Temporal_YISHEN_{block_type}_Layer')
        ...
        for i, [channel, stride, depth] in enumerate(block_args):
            if depth == 1:
                ...
                self.add_module(f'block-{i}_tcn', temporal_layer(channel,channel, temporal_window_size, stride=1, **kwargs))
                self.add_module(f'block-{i}_att', Attention_Layer(channel, **kwargs))
                last_channel = channel
            else:
                self.add_module(f'block-{i}_tcn', temporal_layer(last_channel,channel, temporal_window_size, stride=1, **kwargs))
                self.add_module(f'block-{i}_att', Attention_Layer(channel, **kwargs))
                last_channel = channel
