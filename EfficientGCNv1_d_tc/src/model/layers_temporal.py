...
class Temporal_YISHEN_Mta2Wrapper_Layer(nn.Module):
    """
    继承于 Temporal_Mta2Wrapper_Layer，目的是在Temporal_Mta2Wrapper_Layer的基础上，增加inchanel和outputchannel。
    """
    expansion = 1

    def __init__(self, planes,outchannel, temporal_window_size, act, bias, stride=1, scale=4, stype='normal',
                 foreconv=True, behindconv=True, layer_type="Sep", residual=True, ratio=2, **kwargs):
        """
        convtype: basic bottle  sep  epsep  sg
        """
        super(Temporal_YISHEN_Mta2Wrapper_Layer, self).__init__()
        self.act = act
        self.ratio = ratio
        ...
        logging.info("MtaYISHEN Layer Ratio: {}".format(self.ratio))
        stride = 1
        logging.info("MtaYISHEN Layer stride: {}".format(stride))
        ...
        self.convs3 = nn.Sequential(
            nn.Conv2d(width * scale, outchannel, kernel_size=1, bias=False),
            nn.BatchNorm2d(outchannel)
        ) if foreconv == True else nn.Identity()

        self.downsample = nn.Sequential(
            nn.Conv2d(planes, outchannel, kernel_size=1, bias=False),
            nn.BatchNorm2d(outchannel),
        )

        if not residual:
            self.residual = Zero_Layer()
        elif outchannel != planes:
            self.residual = nn.Sequential(
                nn.Conv2d(planes, outchannel, 1, (stride, 1), bias=bias),
                nn.BatchNorm2d(outchannel),
            )
        elif stride == 1:
            self.residual = nn.Identity()
...
if __name__ == '__main__':
    ...
    model_epseq1_mta = Temporal_MtaWrapper_Layer(planes=32, temporal_window_size=3, bias=True,
                                                 ...
                                                 layer_type="MtaWrapper-T8LiteMBConv",
