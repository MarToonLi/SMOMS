...
class Processor(Initializer):
    ...
    def eval(self):
        ...
        return acc_top1, acc_top5, cm, y_true, y_pred, score
    ...
    def start(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            acc_top1, acc_top5, cm, y_true, y_pred, score = self.eval()
            if acc_top1 > best_state['acc_top1']:
                is_best = True
                best_state.update({'acc_top1': acc_top1, 'acc_top5': acc_top5, 'cm': cm})
                logging.info(cm)
                # np.save(
                #     "/home/bullet/PycharmProjects/EfficientSeries/EfficientGCNv1_d_tc/resources/{}_{}_data.npy".format(
                #         self.args.datasetCode, best_state['acc_top1']), np.array([y_true, y_pred]))
                score_dict = dict(
                    zip(self.eval_loader.dataset.sample_name, score))
                # basepath = "/home/meng/PycharmProjects/EfficientSeries/EfficientGCNv1_d_tc/resources"
                # import pickle
            ...
            print('epoch:{} Best top-1 accuracy: {:.2%}, Total time: {}'.format(epoch,
                best_state['acc_top1'], U.get_time(time() - start_time)
            ))
            logging.info('')
